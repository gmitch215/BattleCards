...
internal class Wrapper1_8_R2 : Wrapper {
    ...
    override fun addPacketInjector(p: Player) {
        ...
        val ch = sp.playerConnection.networkManager.k

        if (ch.pipeline().get(PACKET_INJECTOR_ID) != null) return
        ch.pipeline().addAfter("decoder", PACKET_INJECTOR_ID, PacketHandler1_8_R2(p))

        PacketHandler1_8_R2.PACKET_HANDLERS[p.uniqueId] = handler@{ packet ->
