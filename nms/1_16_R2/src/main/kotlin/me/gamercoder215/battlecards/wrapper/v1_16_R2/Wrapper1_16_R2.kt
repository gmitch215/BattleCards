...
internal class Wrapper1_16_R2 : Wrapper {
    ...
    private fun <T> registry(r: IRegistry<T>): RegistryMaterials<T> {
        return (Bukkit.getServer() as CraftServer).server.aX().b(r.f()) as RegistryMaterials<T>
    }
    ...
    override fun loadProperties(en: Creature, card: IBattleCard<*>) {
        ...
        if (BattleConfig.config.isAggressive)
            nms.targetSelector.a(4, CardNearestAttackableTargetGoal1_16_R2(nms, card))
    ...
    override fun addPacketInjector(p: Player) {
        ...
        ch.pipeline().addAfter("decoder", PACKET_INJECTOR_ID, PacketHandler1_16_R2(p))
