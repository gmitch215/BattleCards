...
internal class Wrapper1_8_R1 : Wrapper {
    ...
    override fun getYBodyRot(entity: LivingEntity): Float = (entity as CraftLivingEntity).handle.aG

    override fun addPacketInjector(p: Player) {
        val sp = (p as CraftPlayer).handle
        val ch = NetworkManager::class.java.getDeclaredField("i").apply { isAccessible = true }.get(sp.playerConnection.networkManager) as Channel

        if (ch.pipeline().get(PACKET_INJECTOR_ID) != null) return
        ch.pipeline().addAfter("decoder", PACKET_INJECTOR_ID, PacketHandler1_8_R1(p))

        PacketHandler1_8_R1.PACKET_HANDLERS[p.uniqueId] = handler@{ packet ->
